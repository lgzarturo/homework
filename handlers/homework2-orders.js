  ...
  helpers.verifyToken(token, email, function(isValid) {
    if (isValid) {
      data.read('payments', order, function(errRead, dataOrder) {
        const message = `
          <p>Reenvio del pago procesado con la tarjeta '${dataOrder.payload.cc}' (${dataOrder.payload.source})</p>
          <h3>Descripción</h3>
          <ul>
        ...
        helpers.mailgun(email, `Reenvio de la Orden #${order}`, message, function(errMailgun) {
          if (!errMailgun) {
