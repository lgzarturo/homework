...
handlers._payments.get = function(data, callback) {
  ...
  let email =
    ...
    data.headers.email.trim().length > 0
      ...
      : false;
  let order =
    typeof data.queryStringObject.order === 'string' &&
    data.queryStringObject.order.trim().length > 0
      ? data.queryStringObject.order.trim()
      : false;
  ...
  _helpers.verifyToken(token, email, function(isValid) {
    if (isValid) {
      _data.read('payments', order, function(err, data) {
        let message = `
                    <p>Reenvio del pago procesado con la tarjeta '${
                      data.payload.cc
                    }' (${data.payload.source})</p>
                    <h3>Descripción</h3>
                    <ul>
                      <li>Cantidad: ${data.payload.items}</li>
                      <li>Total: ${data.payload.amount / 100} ${
...
handlers._payments.post = function(data, callback) {
  ...
  let email =
    ...
    data.headers.email.trim().length > 0
      ...
      : false;
  let creditCart =
    typeof data.payload.creditCart === 'string' &&
    data.payload.creditCart.trim().length > 0
      ? data.payload.creditCart.trim()
      : false;
  let validMonth =
    typeof data.payload.validMonth === 'number' &&
    data.payload.validMonth >= 1 &&
    data.payload.validMonth <= 12
      ? data.payload.validMonth
      : false;
  let validYear =
    typeof data.payload.validYear === 'number' && data.payload.validYear >= 2018
      ? data.payload.validYear
      : false;
  let codeCard =
    typeof data.payload.codeCard === 'string' &&
    data.payload.codeCard.trim().length === 3
      ? data.payload.codeCard
      : false;
          ...
          _data.read('orders', email, function(err, data) {
            if (!err && data) {
              ...
              let payload = {
                ...
                currency: currency,
                description: _helpers.translate(
              ...
              _helpers.stripe(payload, function(err) {
                if (err) {
                  ...
                  let message = `
                                        ...
                                        <p>Se ha procesado el pago con la tarjeta '${
                                          payload.cc
                                        }' (${payload.source})</p>
                                        <h2>Orden #${payload.orderId}</h2>
                                        <ol>${messageItems}</ol>
                                        <h3>Descripción <small>(${
                                          payload.description
                                        })</small></h3>
                                        <ul>
                                          <li>Cantidad: ${payload.items}</li>
                                          <li>Total: ${payload.amount / 100} ${
                    payload.currency
                  }</li>
                                        </ul>
