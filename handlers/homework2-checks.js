                                    ...
                                    if (!err) {
                                        ...
                                        _data.update('users', userEmail, userData, function (err) {
                                            if (!err) {
                                                ...
                                                callback(403, {'error': _helpers.translate('error.user.update.checks', data.lang)});
                                            }
                                        });
                                    } else {
                                        callback(403, {'error': _helpers.translate('error.checks.created', data.lang)});
                                    }
                                });
                            } else {
                                callback(400, {'error': _helpers.translate('error.user.max.checks', data.lang)});
                            }
                        } else {
                            callback(403);
                        }
...
handlers._checks.get = function (data, callback) {
    // Validar los parámetros de la solicitud.
    let id = typeof (data.queryStringObject.id) === 'string' && data.queryStringObject.id.trim().length === 48 ? data.queryStringObject.id.trim() : false;
    if (id) {
...
handlers._checks.put = function (data, callback) {
    ...
    if (id) {
        if (protocol || url || method || successCodes || timeoutSeconds) {
                        ...
                        if (isValid) {
                            ...
                            _data.update('checks', id, checkData, function (err) {
                                if (!err) {
                                    ...
                                    callback(409, {'error': _helpers.translate('error.record.updated', data.lang)});
                                }
                            });
            ...
            callback(400, {'error': _helpers.translate('error.check.missing.fields', data.lang)});
        }
                            ...
                            if (!err) {
                                _data.read('users', checkData.userEmail, function (err, userData) {
                                    if (!err && userData) {
                                        ...
                                        callback(409, {'error': _helpers.translate('error.check.user.found', data.lang)});
                                    }
                                });
                            } else {
                                callback(409, {'error': _helpers.translate('error.record.delete', data.lang)});
                            }
                        });
