...
handlers.checks.post = function(req, callback) {
  ...
  const protocol = typeof req.payload.protocol === 'string' && ['http', 'https'].indexOf(req.payload.protocol) > -1 ? req.payload.protocol : false;
  const url = typeof req.payload.url === 'string' && req.payload.url.trim().length > 0 ? req.payload.url.trim() : false;
  const method = typeof req.payload.method === 'string' && ['get', 'post', 'put', 'delete'].indexOf(req.payload.method) > -1 ? req.payload.method : false;
