...
// @ignore
let _checks = require('./../handlers/homework2-checks');
// @ignore
 ...
 */
lib.httpsServer = https.createServer(lib.httpsServerOptions, function(
  req,
  res
) {
  lib.unifiedServer(req, res);
...
lib.unifiedServer = function(req, res) {
  ...
  req.on('end', function() {
    ...
    let chooseHandler =
      typeof lib.router[trimmedPath] !== 'undefined'
        ? lib.router[trimmedPath]
        : _homework1.notFound;
    ...
    chooseHandler(data, function(statusCode, payload) {
      ...
      let debugColor =
        statusCode === 200 ? '\x1b[32m%s\x1b[0m' : '\x1b[31m%s\x1b[0m';
 ...
 */
lib.router = {
  ping: _homework1.ping,
  hello: _homework1.hello,
  pizza: _homework2.pizza,
  'api/users': _users.users,
  'api/tokens': _tokens.tokens,
  'api/checks': _checks.checks,
  'api/menu': _menu.items,
  'api/shopping-cart': _shopping.cart,
  'api/payments': _orders.payments
};
...
lib.init = function() {
  ...
  lib.httpServer.listen(_config.httpPort, function() {
    console.log(
      '\x1b[36m%s\x1b[0m',
      `The server HTTP is listening on port ${_config.httpPort} in [${_config.envName}] mode`
    );
  });
  ...
  lib.httpsServer.listen(_config.httpsPort, function() {
    console.log(
      '\x1b[35m%s\x1b[0m',
      `The server HTTPS is listening on port ${_config.httpsPort} in [${_config.envName}] mode`
    );
  });
};
