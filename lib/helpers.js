...
lib.applyLayout = (str, data, callback) => {
  str = validations.isValidTextField(str)
  data = clean.getValidObject(data)

  lib.getTemplate('layouts/main', data, (errLayout, layoutString) => {
    if (!errLayout && layoutString) {
      lib.getTemplate('partials/_header', data, (errHeader, headerString) => {
        if (!errHeader && headerString) {
          lib.getTemplate('partials/_footer', data, (errFooter, footerString) => {
