...
lib.hash = function(str) {
  if (typeof str == 'string' && str.length > 0) {
    return crypto
      .createHmac('sha256', _config.hashingSecret)
      .update(str)
      .digest('hex');
  }
  return false;
...
lib.createRandomString = function(strLength) {
  ...
  if (strLength) {
    let posibleCharacters =
      'aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789';
    let str = '';
    for (let i = 1; i <= strLength; i++) {
      let randomCharacter = posibleCharacters.charAt(
        Math.floor(Math.random() * posibleCharacters.length)
      );
      str += randomCharacter;
    }
    return str;
...
lib.twilio = function(phone, msg, callback) {
  ...
  if (phone && msg) {
    ...
    callback('Los parametros proporcionados no son correctos');
  }
};
